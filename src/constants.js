...
/**
 const items = eventData.map((item, index)=>{
    const hdr = this._renderHeader(false,item.icon, COMMON_ICON_STYLE, item.titleText, item.displayText, item.displayText,null );
    const cntnt = this._renderContent(item);
    return <View><View style={styles.editHeader}>{hdr}</View><View style={styles.hiddenEdit}>{cntnt}</View>;
  });
    
  console.log("::EventViewComponents items::", items);
...
export const getDefaultEvent = (eventObject) => {
  ...
  event.calendar = { year: '2021', month: '01', day: '01', hour: '00', minute: '00'};
  return event;
};
